...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="large-v2",
        choices="tiny.en,tiny,base.en,base,small.en,small,medium.en,medium,large-v1,large-v2,large,large-v3".split(
            ","
        ),
    ...
    if args.offline and args.comp_unaware:
        logger.debug(
            f"No or one option from --offline and --comp_unaware are available, not both. Exiting."
        )
        sys.exit(1)
    ...
    logger.debug(f"Audio duration is: %2.2f seconds {duration}")

    size = args.model
    language = args.lan

    t = time.time()
    logger.debug(f"Loading Whisper {size} model for {language}...", end=" ")
    ...
    if args.vad:
        logger.debug(f"setting VAD filter")
        asr.use_vad()
